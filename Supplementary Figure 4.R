...
#### Load in csv file ####
Data.NotNormalised <- read.csv(file = paste0(Data.location, "Supplementary Figure 4.csv"))
Data.Normalised <- read.csv(file = paste0(Data.location, "Supplementary Figure 4 Normalised.csv"))
...
#### Plotting ####
# Cell Count Box plot
ggplot(Data.NotNormalised, aes(x = Polymer, y = Cell.Count)) +
  ...
  scale_fill_manual(values = c("#FBB4AE", "#FFCCFF", "#B3CDE3", "#CCEBC5")) + 
  geom_boxplot(aes(fill = Selected.iTEC), lwd=0.25) +
  labs(
    title = "Un-normalised Murine Native TEC Focused Array",
    y = "Cell Count",
    x = "Polymer",
    fill = "") 

ggsave(filename = "Supplementary Figure 4 Un-normalised.tiff", device = "tiff", 
       path = Graph.Output.location, width = 147.9973, height = 74.93, units = "mm",
       dpi = 2400)

# Unnormalised Box plot by replicate
ggplot(Data.NotNormalised, aes(x = Replicate, y = Cell.Count)) +
...
ggsave(filename = "Supplementary Figure 4 Un-normalised by replicate.tiff", device = "tiff", 
       path = Graph.Output.location, width = 147.9973, height = 74.93, units = "mm",
       dpi = 2400)

# Normalised Box plot by replicate
ggplot(Data.Normalised, aes(x = Replicate, y = Normalised.Counts)) +
...
ggsave(filename = "Supplementary Figure 4 Normalised by replicate.tiff", device = "tiff", 
       path = Graph.Output.location, width = 147.9973, height = 74.93, units = "mm",
       dpi = 2400)
